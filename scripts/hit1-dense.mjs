...
const DIR = "C:/Users/XiongT/.claude/jobs/2d345c50/tmp/hit1/dense";
const TIMES = Array.from({ length: 33 }, (_, i) => Math.round((0.083 + i / 6) * 100) / 100);
...
const pick =
  players.find((p) => /white shirt|cap|hat|green short/i.test(p.label)) ?? players[0];
console.log(`\nPICKED: ${pick.label}`);
...
await writeFile(path.join(ROOT, "ab", "hit1-dense-ringed.jpg"), ringedBuf);
