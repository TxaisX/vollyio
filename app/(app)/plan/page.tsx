...
// The plan is generated by a server action, and a server action inherits its
// route segment's config rather than declaring its own. Without this line the
// generation ran against whatever the platform's undeclared default happens to
// be, while its own worst case is three 50 second attempts plus backoff.
//
// The gateway is why that worst case is real rather than theoretical: it
// answers 200 within a second and then pads the body until the upstream
// finishes, and measured draws were still padding at 60 and 170 seconds. Those
// do not recover if given longer, which is what makes a short ceiling with
// retries the right shape and a long single wait the wrong one.
//
// A platform kill is survivable but not free: `reserve_weekly_plan` has already
// claimed the row, so the player waits out the ten minute claim expiry before
// they can try again (D-072). 180 leaves headroom over the action's own budget
// so the timeout that fires is ours, with a message, rather than the platform's.
