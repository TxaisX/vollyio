-- 048: Pro is 24 a month (D-085).
--
-- Four reads of every skill the product scores, on the same logic the signup
-- grant uses: SKILLS.length is 6, so 6 is one of each and 24 is four of each.
-- 18 was a round number with nothing behind it; 24 is a sentence a player can
-- be told, and it puts a weekly rep on every skill within reach.
--
-- Economics at the measured $0.234, unchanged price of $9.99 (nets $9.40):
-- full use costs $5.62 and clears $3.78, a 40% margin against 55% at 18.
-- Full use is the worst case and not the norm. Break-even conversion at full
-- use moves to ~5.8% and at half use is ~3.3%, both inside the band D-076
-- established. docs/decisions.md D-085 records the price recommendation that
-- goes with this and why it was NOT bundled into the same change.
--
-- Restated whole for the reason 044 through 047 record.
