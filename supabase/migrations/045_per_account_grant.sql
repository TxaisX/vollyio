-- 045: the grant becomes a number the owner can set per account (D-082).
--
-- Early users recruited by hand need more room than the standard grant buys,
-- and the only lever available was a global number that changes it for
-- everyone. `profiles.analysis_grant` overrides `signup_grant()` for ONE
-- account: null means the standard grant, so an untouched account is
-- unaffected by construction rather than by a branch anyone has to maintain.
--
-- It is spent exactly the way the standard grant is: coalesce feeds the same
-- `greatest(0, v_grant - v_lifetime)`, counted against LIFETIME rows in
-- `analyses`, stored nowhere and decremented never. So it inherits D-076's
-- properties whole: nothing to drift, nothing to farm, and a failed clip
-- still costs the player nothing.
--
-- ENTITLEMENT IS NOT PLAYER-WRITABLE (security.md rule 11). The column is
-- outside migration 012's fixed update allowlist by construction, the setter
-- below is service_role only like `set_subscription_plan`, and
-- `lib/plans.test.ts` asserts across every migration that no later one hands
-- the column to `authenticated`.
--
-- The ceiling is 500 and it is not decoration. Every analysis is a real paid
-- coaching call at roughly $0.234, so an extra digit typed into the setter is
-- a real bill. 500 is far past any legitimate hand-out and well short of the
-- number that would hurt.
--
-- Restates the whole allowance shape from 044 for the reason 044 records:
-- lib/plans.test.ts resolves the NEWEST migration defining
-- plan_monthly_allowance and asserts every pin against that one file.
