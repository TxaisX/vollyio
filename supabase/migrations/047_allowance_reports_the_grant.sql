-- 047: the allowance reports the grant's own numbers (D-084).
--
-- `analysis_allowance()` returned `allowance`, derived as
-- `greatest(rate, least(grant_left + used, grant))`. That is correct
-- arithmetic and a dishonest denominator: an account handed a grant of 50
-- with 20 analyses behind it and 1 this window renders "30 of 31 left this
-- month", where 31 is an artifact of the sum and the 30 do not reset the way
-- "this month" claims. Two false things in one sentence, on the one surface
-- whose whole job is telling a player where they stand.
--
-- The fix is data, not phrasing: report `grant` and `lifetime_used` and let
-- the copy say "20 of 50 used" while the grant is what is being spent, and
-- go back to the window frame once the recurring rate is what binds.
-- Both fields are additive; every existing caller reads the same keys it
-- always did, and `lib/allowance.ts` treats their absence as "cannot say"
-- rather than inventing a lifetime.
--
-- The gate is untouched. Nothing here changes what any player may spend.
-- Restated whole for the reason 044 through 046 record.
