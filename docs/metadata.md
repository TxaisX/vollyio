...
6. Public-library indexability: `/drills`, `/drills/[slug]`, `/learn`, `/learn/[skill]`, `/learn/rehab` and `/learn/rehab/[slug]` all sit inside `(app)/layout.tsx`, which sets `robots: { index:false }`. Every one of them must override with `robots: { index:true, follow:true }` in its own metadata. **This note was written for the two drills routes, they were fixed, and the four learn routes were missed for months**: a live probe on 2026-08-25 found 41 of the 104 URLs in `sitemap.xml` serving noindex, the whole injury library among them. `lib/public-indexing-contract.test.ts` now fails the build instead of leaving this to a note. Whether the routes are actually reachable without auth is a `proxy.ts` decision for Dave; the metadata override is noted here so it is not missed. All other app routes stay noindex.
