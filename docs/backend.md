...
- **Model routing (D-004 / CS-5).** Cheapest capable tier per call: coach chat is high-frequency → `COACH_MODEL` (fast conversational tier); analyze is low-frequency, vision + structured scoring → `ANALYZE_MODEL` (top reasoning tier) (`client.ts:6-7`). Both are checked-in server-side constants, not environment variables — a typo'd model string should fail review, not surface as a runtime 404 on a paid call path. The persisted `analyses.model` records which ran (or `"mock"`). See `decisions.md` D-004.
- **Effort routing (D-027).** Reasoning depth is pinned per tier alongside the model: `ANALYZE_EFFORT` (low — the top tier's effort ladder is flat, so low buys the same accuracy at roughly half the wall clock) and `COACH_EFFORT` (medium — a cap, since the conversational tier defaults to high and fabricated detail above medium). Analyze also sets adaptive thinking explicitly: with the parameter absent, the top tier runs with no reasoning at all and effort controls nothing. See `decisions.md` D-027.
