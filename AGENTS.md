...
- No vendor names in UI, docs, or user-visible errors. The AI layer is referred to as "the coaching service" and, for the frame read, "the vision provider"; the only vendor-named strings in the repo are the `ANTHROPIC_API_KEY` and `OPENROUTER_API_KEY` env vars, the model constants in `lib/ai/client.ts`, and the gateway endpoint in `lib/ai/vision.ts` (D-093), all server-side only.
