...
- No vendor names in UI, docs, or user-visible errors. The AI layer is referred to as "the coaching service". Vendor-named strings survive in exactly three places, all server-side: the `OPENROUTER_API_KEY` env var, the two model ids in `lib/ai/client.ts`, and the endpoint URL in `lib/ai/vision.ts` and `lib/ai/chat.ts`. Nothing else, and nothing a player can see.
- Design tokens live in `app/globals.css` `@theme` (navy/chalk/gold/teal; Space Grotesk display, Instrument Sans body, IBM Plex Mono labels) — never introduce new colors or fonts. Within those tokens, expressive use is house style, not an exception (D-123): gradients, inset light, resting shadows, and ambient washes built from mixes of the standing colors need no grant. Shared component classes (`card`, `btn-primary`, `btn-ghost`, `chip`, `input-field`, `spot`, `reveal`, `text-sheen`, `app-atmosphere`) live there too — reuse them, don't reinvent per page.
