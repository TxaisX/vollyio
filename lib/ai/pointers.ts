...
export function pointerSpec(skill: Skill): string {
  const lines: string[] = [
    ...
    '- "partial": present but flawed or inconsistent across reps.',
    '- "missed": the athlete visibly does not do it.',
    '- "not_visible": the footage genuinely does not show it (occluded, cropped, cut, blurred). Never use not_visible for a mechanic you can see; never use missed for one you cannot.',
